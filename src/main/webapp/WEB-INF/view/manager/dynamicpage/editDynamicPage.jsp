...
<form:form method="POST" action="/managercms/dynamiccontent/savedcontent.html" modelAttribute="dynamicContent">
    ...
    <div id="redirURL" style="display: none;">
        ...
        <div>
            <form:select path="redirectUrl" items="${dynamicContent.redirectUrl}" multiple="true" size="3"
                         style="height:100%;min-width: 330px;"/>
        </div>
        <div><input type="button" id="butAddUrl" value="Добавить URL"/> <input type="button" id="butRemUrl"
                                                                               value="Удалить выделенные URL"/></div>
        <div id="winEnterUrl">
            <div>
    ...
    <div>
        ...
        <script type="application/javascript">
            var fLC = {
                ...
                init: function () {
                    $('#typeDynCont').change(function () {
                        fLC.loadtypetemplate();
                    });
                    $('#cancalEdit').click(function () {
                        window.history.back();
                        return false;
                    });
                    $('#shName').change(function () {
                        fLC.loadconteiners()
                    });
                    ...
                    $('#butAddUrl').click(function () {
                        jQuery("#winEnterUrl").dialog("open");
                        return false;
                    });
                    $('#butRemUrl').click(function () {
                        jQuery("#redirectUrl option:selected").remove();
                        return false;
                    });
                    $('#dynamicContent').submit(function () {
                        jQuery("#redirectUrl option").attr('selected', 'selected');
                    });
    ...
    <div>
        ...
        <button name="Отмена" value="Отмена" id="cancalEdit"> Отмена</button>
    </div>
</form:form>