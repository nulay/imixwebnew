...
<div>
    <table class="rttable tableredirect">
        ...
        <c:forEach items="${allredirect}" var="curentred">
            <tr>
                <td><input class='idRed' type='hidden' value='<c:out value="${curentred.id}"/>'/><c:out
                        value="${curentred.url}"/></td>
                <td><c:out value="${curentred.id_nodeView}"/></td>
                <td><input type='checkbox' class='chSel'/></td>
            </tr>
        </c:forEach>
        </tbody>
    </table>
